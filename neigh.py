...
# datasets = [("OClinks_w.txt","d","w"),("celegans_n306.txt","d","w"),("USairport500.txt","d","w")
#             ,("openflights.txt","d","w"),("USairport_2010.txt","d","w"),
#     ("Freemans_EIES-3_n32.txt","d","w"),("lkml.txt","d","w"),
    ...
    for source_node in G.vs:
        ...
        for node in G.vs:
            if node!=source_node:
                ...
                # inf +=  np.exp(source_node['h2-idx']-node['h2-idx']) / dist #check
                inf += node['h2-idx']/dist

        source_node['heuristic'] = np.exp(source_node['h2-idx']/len(G.vs)) * inf
        # source_node['heuristic'] = np.exp(source_node['h2-idx']) * inf
    ...
    for source_node in G.vs:
        ...
        for node in G.vs:
            if node!= source_node:
                ...
                # common_ne = list(set(G.neighbors(source_node,'out')).intersection(G.neighbors(node,'out'))) #get common out neighbours
                # union_ne  = list( set ( G.neighbors(source_node,'out') + G.neighbors(node,'out') ) )
                
                # if len(union_ne) == 0:
                #     # print("n")
                #     prob = 0.01
                # else:
                #     prob = 1-(len(common_ne)/len(union_ne))

                inf += (np.exp(source_node['h2-idx']-node['h2-idx'])/dist) * np.exp(node['h2-idx'])

        source_node['neigh_heur'] = inf
...
def Profit_Leader(G):
    ...
    for source_node in G.vs:
        ...
        ar = 0
        for target_node in G.neighbors(source_node,'out'):
            ar += d

        common_ne = list(set(G.neighbors(u,'out')).intersection(G.neighbors(v,'in'))) #get common neighbours: successor of u and predecessor of v
...
# %%
methods = ['GSM','heuristic','neigh_heur']
...
for dataset,directed,weighted in tqdm(datasets):
    ...
    for method in methods:
        ...
        for i in range(seed_size):
            ...
            seeds.append(seed)
            df_result.at[i,method] =  ([seed.index for seed in seeds],get_inf(seeds,no_iterations))
...
# %%
# df[['Node','out_core','p=Original']]

# %%
# np.unique(G.vs['shell']),np.unique(G.vs['h2-idx'])
